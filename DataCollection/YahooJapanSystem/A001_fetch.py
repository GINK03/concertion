...
try:
    ...
    from Web import GyotakuPhantomJs
...
def process(arg):
    try:
        ...
        blobs_digest = GyotakuPhantomJs.gyotaku(url)
        score = 1/(arg.rank+1)

        url_digest = GetDigest.get_digest(url)
        now = datetime.datetime.now()
        title_url_digest_score = TitleUrlDigestScore(title=title, url=url, digest=blobs_digest, score=score, date=now, category=category)
...
def fetch_each_categories():
    ...
    for category, url in RANKING_URLs:
        ...
        args = [ARG(category=category,
                    ...
                    url=a.get('href'),
                    rank=idx) for idx, a in enumerate(soup.find_all('a', {'class': 'newsFeed_item_link'}))]

        try:
            with ThreadPoolExecutor(max_workers=8) as exe:
                for ret in exe.map(process, args, timeout=180):
                    ret
